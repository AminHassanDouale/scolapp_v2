...
/**
 * Reusable profile form (avatar, info, password, preferences).
 *
 * This is a NESTED component so it can be embedded inside any role's layout
 * (admin, teacher, caissier, guardian, student, monitor) — that keeps the
 * user's own portal menu around them instead of dropping them into the admin
 * chrome, which previously left non-admins with no way to navigate away.
 */
new class extends Component {
    ...
    /** The profile route that matches the current user's role (keeps their layout). */
    public static function profileRouteForUser(): string
    {
        $user = auth()->user();
        return match (true) {
            (bool) $user?->hasRole('teacher')  => 'profile.teacher',
            (bool) $user?->hasRole('caissier') => 'profile.caissier',
            (bool) $user?->hasRole('monitor')  => 'profile.monitor',
            (bool) $user?->hasRole('guardian') => 'profile.guardian',
            (bool) $user?->hasRole('student')  => 'profile.student',
    ...
    public function switchLang(string $lang): void
    {
        ...
        session(['locale' => $lang]);
        // Full reload of the SAME role's profile page so menus + content re-render
        $this->redirect(route(self::profileRouteForUser()), navigate: false);
    }
...
<div>
    <x-header title="Mon profil" separator />
                    ...
                    <x-card class="mt-4">
                        ...
                        <div class="flex items-center justify-between p-4 bg-base-200 rounded-xl">
                            <div>
                                ...
                                <p class="text-sm text-base-content/60">Réinitialisez votre mot de passe de connexion</p>
                            </div>
                ...
                <x-tab name="prefs" label="Préférences" icon="o-cog-6-tooth">
                    <x-card class="mt-4 space-y-6">
                        ...
                        <div x-data="{ darkMode: localStorage.getItem('theme') === 'dark' }">
                            ...
                            <div class="flex gap-3">
                                <button type="button" @click="darkMode = false; localStorage.setItem('theme', 'light'); document.documentElement.setAttribute('data-theme', 'light')"
                                        :class="!darkMode ? 'border-primary bg-primary/10 font-bold' : 'border-base-300'"
                                        class="flex-1 py-3 px-4 rounded-xl border-2 transition-all text-center">
                                    ☀️ Clair
                                </button>
                                <button type="button" @click="darkMode = true; localStorage.setItem('theme', 'dark'); document.documentElement.setAttribute('data-theme', 'dark')"
                                        :class="darkMode ? 'border-primary bg-primary/10 font-bold' : 'border-base-300'"
                                        class="flex-1 py-3 px-4 rounded-xl border-2 transition-all text-center">
                                    🌙 Sombre
                                </button>

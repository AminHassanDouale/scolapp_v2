...
new #[Layout('layouts.teacher')] class extends Component {
    ...
    {
        $teacher = Teacher::where('user_id', auth()->id())->with('schoolClasses')->first();
        ...
        return compact('classes', 'students', 'teacher');
    }
};
?>

<div class="p-4 lg:p-6 space-y-6">
    <x-header title="{{ __('navigation.students') }}" subtitle="{{ __('navigation.my_students') }}" separator>
        <x-slot:actions>
    ...
    <x-card shadow class="border-0">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <x-select wire:model.live="selectedClassId" placeholder="Toutes mes classes"
                    ...
                    <td>
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center">
                                <span class="text-xs font-bold text-indigo-700">{{ substr($student->name, 0, 1) }}</span>
                            </div>

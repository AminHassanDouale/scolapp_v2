...
new #[Layout('layouts.empty')] class extends Component {
    ...
    public function mount(string $uuid): void
    {
        ...
        $this->invoice = Invoice::where('uuid', $uuid)
...
<div>
    ...
    <div class="no-print sticky top-0 z-50 bg-white border-b border-slate-200 px-6 py-3 flex items-center gap-3 shadow-sm">
        <a href="{{ route('guardian.invoices') }}" class="btn btn-ghost btn-sm gap-2">
        ...
        <div class="flex-1 flex items-center gap-2">
            ...
            <span class="badge badge-{{ $invoice->status?->color() }} badge-sm">{{ $invoice->status?->label() }}</span>
        </div>
    ...
    <div class="invoice-paper relative">
        ...
        {{-- Header --}}
        <div style="background:linear-gradient(135deg,#064e3b 0%,#065f46 55%,#059669 100%);padding:28px 32px 24px;color:white;position:relative;overflow:hidden;">
        ...
        <div style="background:#f8fafc;border-bottom:1px solid #e2e8f0;padding:14px 32px;">
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;">
                ...
                <div>
                    ...
                    <p style="font-weight:700;color:{{ $overdue ? '#dc2626' : '#1e293b' }};margin:0;font-size:0.95rem;">{{ $val }}{{ $overdue ? ' ⚠' : '' }}</p>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div style="padding:24px 32px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;">
                ...
                <div style="background:linear-gradient(135deg,#ecfdf5,#d1fae5);border:1px solid #a7f3d0;border-radius:12px;padding:16px;">
                    <p style="font-size:0.62rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#059669;margin:0 0 8px;">Élève</p>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:0.88rem;margin-bottom:24px;">
                <thead>
                    <tr style="background:linear-gradient(90deg,#064e3b,#059669);color:white;">
                ...
                <tfoot>
                    ...
                    @endif
                    <tr style="background:linear-gradient(135deg,#ecfdf5,#d1fae5);border-top:2px solid #a7f3d0;">
                        <td colspan="{{ $invoice->installment_number ? 2 : 1 }}" style="padding:13px 14px;text-align:right;font-weight:800;color:#064e3b;text-transform:uppercase;letter-spacing:0.06em;font-size:0.85rem;">Total</td>
                        <td style="padding:13px 14px;text-align:right;font-weight:900;font-size:1.2rem;color:#064e3b;">{{ number_format((int)$invoice->total, 0, ',', ' ') }} DJF</td>
                    </tr>
            ...
            <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:16px;margin-bottom:24px;">
                <p style="font-size:0.65rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#94a3b8;margin:0 0 10px;">Historique des paiements</p>
                <table style="width:100%;border-collapse:collapse;font-size:0.82rem;">
                    <thead>
                        <tr style="border-bottom:1px solid #e2e8f0;">
                            ...
                            <th style="text-align:center;padding:4px 8px;font-size:0.72rem;color:#64748b;">Statut</th>
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom:1px solid #f1f5f9;">
                            <td style="padding:6px 8px;font-family:monospace;color:#64748b;font-size:0.78rem;">{{ $alloc->payment->reference }}</td>
                            <td style="padding:6px 8px;text-align:center;color:#64748b;">{{ $alloc->payment->payment_date?->format('d/m/Y') }}</td>
                            <td style="padding:6px 8px;text-align:center;color:#64748b;text-transform:capitalize;">{{ $alloc->payment->notes ?? $alloc->payment->payment_method }}</td>
                            <td style="padding:6px 8px;text-align:center;">
                                @php $pstatus = $alloc->payment->status; @endphp
                                <span style="font-size:0.7rem;font-weight:600;padding:2px 8px;border-radius:20px;
                                    background:{{ $pstatus === \App\Enums\PaymentStatus::CONFIRMED ? '#dcfce7' : ($pstatus === \App\Enums\PaymentStatus::PENDING ? '#fef9c3' : '#fee2e2') }};
                                    color:{{ $pstatus === \App\Enums\PaymentStatus::CONFIRMED ? '#166534' : ($pstatus === \App\Enums\PaymentStatus::PENDING ? '#854d0e' : '#991b1b') }};">
                                    {{ $pstatus?->label() }}
                                </span>
                            </td>
            ...
            <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:12px 16px;margin-bottom:24px;" class="print-only">
            ...
            <div class="print-only" style="margin-top:32px;">
            ...
            <div style="border-top:1px solid #e2e8f0;padding-top:14px;margin-top:24px;display:flex;align-items:flex-end;justify-content:space-between;">
                <div style="font-size:0.7rem;color:#94a3b8;line-height:1.7;">
                    ...
                    <p style="margin:0;">Imprimé le {{ now()->format('d/m/Y à H:i') }}</p>
                </div>
                <div style="display:flex;align-items:center;gap:5px;opacity:0.45;">
                    <span style="font-size:0.7rem;font-weight:700;color:#059669;letter-spacing:0.05em;">ScolApp SMS</span>
                </div>
            </div>
        </div>
    </div>
